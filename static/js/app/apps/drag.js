...
ToolMan._junkdrawer = {
	...
	restoreListOrder : function(listID) {
		...
		var IDs = cookie.split('|'),
			items = ToolMan.junkdrawer()._itemsByID(list);
...
_ToolManDragGroup.prototype = {
	...
	onTopWhileDragging : function(zIndex) {
		...
		this.register('dragend', function(dragEvent) {
			if(originalZIndex)dragEvent.group.element.style.zIndex = originalZIndex
		})
	},
...
ToolMan._dragsortFactory = {
	 ...
	 */
	//初始化ul列表拖拽排序
	makeListSortable : function(list, callback) {
...
$.registerNameSpace("bf.app.apps.drag");

var dragsort = ToolMan.dragsort(),
	junkdrawer = ToolMan.junkdrawer();
//拖拽组件
$.bf.app.apps.drag.app_drag = function(id,triggerFun)
{
	//根据cookie中的序号重置列表
	junkdrawer.restoreListOrder(id);
	//初始化ul列表拖拽排序
	dragsort.makeListSortable(document.getElementById(id), triggerFun);
}