...
define('UCLIB_LOG_RPC_TYPE_RPC_SERVER', 'rpc_server'); 
define('UCLIB_LOG_RPC_TYPE_CAS', 'cas'); 
define('UCLIB_LOG_RPC_TYPE_FEED', 'feed'); 
...
{ 
    ...
    */
    ...
    public function notify($msg, $db, $pri, $params)
    {
        if( !strstr($msg, 'Error') && mt_rand(0, 10000) > $this->_rate )
            return; 
...
class PageProfileLogOb extends UCLibLogObserver {
	...
	public function getProfileTable() {
		$html = array();
		$html[] = '<div style="float:left; clear:both;"><table>';
		$html[] = <<<EOM
	<tr>
		<td width="200">模块名</td>
...
EOM;
		foreach ($this->_stats as $log) {
			$html[] = $this->_printTableRow($log);
		}
		$html[] = '</table></div>';
	...
	private function _analyzeLog($msg) {
		$result = preg_match_all('/\[([^\]]*)\]/', $msg, $matches, PREG_OFFSET_CAPTURE);
		if (!$result or count($matches[1]) < 2) return false;
		$method = $matches[1][0][0];
		$type = $matches[1][1][0];
		$offset = $matches[1][1][1];
	
		$result = preg_match('/<(\S*)>\s(\S*?)\s(\S*?)\s#/', $msg, $matches, null, $offset);
		if (!$result) return false;
		$exe_time = (float)$matches[1];
		$api = $matches[2];
		$status = $matches[3];
	
		return array('method'=>$method, 'type'=>$type, 'exe_time'=>$exe_time, 'api'=>$api, 'status'=>$status);
	}
	
	private function _printTableRow($data) {
		$html = array();
		$html[] = '<tr>';
		$html[] = "<td>{$data['api']}</td>";
		$html[] = "<td>{$data['exe_time']}</td>";
